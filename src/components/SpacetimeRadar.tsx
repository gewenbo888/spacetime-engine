...
export default function SpacetimeRadar() {
  const { lang } = useLang();
        ...
        <div key={axis ?? -1} className="mt-6 min-h-[92px] rounded-xl border border-flux-500/20 bg-void-900/60 p-4 lang-fade">
          {axis === null ? (
            <p className="text-sm leading-relaxed text-ink-400">
              <T v={{ en: "Hover an axis to read what it measures. Each polygon is how one regime of physics treats the seven ingredients of spacetime structure.", zh: "悬停某个轴以阅读它度量什么。每个多边形，是某种物理区域处理时空结构七种成分的方式。" }} />
            </p>
          ) : (
            <>
